...
核对日期：2026-09-11，Remotion 入口与目录数于 2026-09-22 复核。先按任务选择一个主 Skill，只读相关参考。项目业务知识在 Git 内维护；通用个人工具与插件保留原安装，不重复复制进项目。

## 项目维护的 21 个入口

以下文件均在 `.agents/skills/`，当前实际为 **21 个目录**（2026-09-22 复核一致）。16 个贝强专用个人 Skills 已整包迁入，迁移前后 50 个文件、130,809 字节的 SHA-256 全部一致，原全局入口已移除；下列目录是后续维护位置。
...
- Remotion 项目参考来自 4.0.522，2026-09-11 检查的插件内参考为 4.0.506。本地较新知识已保留。
- **11 个重复 Remotion 顶层入口（2026-09-22 实际移除）**：2026-09-11 的记录曾称这批重复入口已移除，但 09-22 复核发现它们仍在 `.agents/skills/` 下残留，每个只有一个 Remotion 图标 PNG、没有 SKILL.md、Git 也未跟踪。已移入 `99_临时区/待清理_2026-09-22/remotion顶层空壳/`。`remotion-best-practices/SKILL.md` 只引用其**自身内部**的 11 个模式参考（`remotion-best-practices/remotion-*/REFERENCE.md`），与这些顶层空壳无关，因此删除不影响调用。插件升级会再次重建同名空壳，升级后需重新核对目录数是否回到 21。
