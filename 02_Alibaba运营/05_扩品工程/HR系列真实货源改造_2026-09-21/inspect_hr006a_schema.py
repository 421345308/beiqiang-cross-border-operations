"""Read-only HR006-A product/schema inventory for the planned 5838 migration."""
...
OUTPUT = Path(__file__).with_name("HR006-A_5838_schema_inventory.json")
PRODUCT_ID = 1601943439124
...
wanted_attribute_values = {
    "rotary dial", "dial closure", "knit", "knitted", "mesh", "textile",
    ...
    for field in sale_holder.findall(".//field"):
        ...
        for option in options:
            ...
            if name == "EUR Size" and display not in {str(value) for value in range(39, 45)}:
                continue
            if name == "Color" and (display or "").lower() not in {"beige", "blue", "black"}:
                continue
