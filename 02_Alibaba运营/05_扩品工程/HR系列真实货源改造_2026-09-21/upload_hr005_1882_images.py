"""Plan or upload the audited HR005/1882 image set to Alibaba photobank.

Default mode is read-only. Use --apply only after HR004 passes public QA.
"""
...
ASSET_ROOT = ROOT / "99_临时区" / "HR005_1882"
GALLERY = ASSET_ROOT / "02_确定性排版候选"
SKU_DIR = ASSET_ROOT / "03_SKU上传候选"
HR002_COMPANY = ROOT / "99_临时区" / "HR002_S6077" / "04_公司图候选"
HR002_MANIFEST = PROJECT / "HR002_S6077_图片银行映射.json"
RECEIPT = PROJECT / "HR005_1882_图片银行映射.json"

FILES = [
    ("main", 1, GALLERY / "M1_HERO_1882.jpg"),
    ("main", 2, GALLERY / "M2_COLORS_1882.jpg"),
    ("main", 3, GALLERY / "M3_MESH_1882.jpg"),
    ("main", 4, GALLERY / "M4_BACKLESS_1882.jpg"),
    ("main", 5, GALLERY / "M5_SIZE_1882.jpg"),
    ("main", 6, GALLERY / "M6_FACTORY_1882.jpg"),
    ("detail", 1, GALLERY / "D1_OVERVIEW_1882.jpg"),
    ("detail", 2, GALLERY / "D2_GRAY_1882.jpg"),
    ("detail", 3, GALLERY / "D3_BLACK_1882.jpg"),
    ("detail", 4, GALLERY / "D4_ORDER_1882.jpg"),
    ...
    ("sku", 1, SKU_DIR / "SKU_GREY_1882.jpg"),
    ("sku", 2, SKU_DIR / "SKU_BLACK_1882.jpg"),
]
...
def refs(client, response):
    return {"request_id": response.get("request_id"), "trace_id": response.get("_trace_id_") or response.get("trace_id"), "error": client.api_error(response)}
...
def main():
    ...
    existing = load_json(RECEIPT, {"product_family": "HR005", "source_model": "1882", "assets": []})
    prior_rows = list(existing.get("assets", []))
    if HR002_MANIFEST.exists():
        prior_rows.extend(load_json(HR002_MANIFEST, {}).get("assets", []))
    reusable = {row.get("sha256"): row for row in prior_rows if row.get("sha256") and row.get("url") and row.get("file_id")}
    plan = []
    for role, slot, path in FILES:
        sha = digest(path)
        prior = reusable.get(sha)
        plan.append({
            "role": role, "slot": slot, "file": str(path), "name": path.name,
            "bytes": path.stat().st_size, "sha256": sha, "status": "reuse" if prior else "upload",
            "reused_url": prior.get("url") if prior else None,
            "reused_file_id": prior.get("file_id") if prior else None,
        })
    if not args.apply:
        print(json.dumps({
            "mode": "plan", "count": len(plan),
            "upload_count": sum(item["status"] == "upload" for item in plan),
            "reuse_count": sum(item["status"] == "reuse" for item in plan), "assets": plan,
    ...
    for item in plan:
        ...
        if prior:
            rows.append({**item, "status": "reused", "url": prior["url"], "file_id": prior["file_id"], "error": None})
            continue
        last = None
        for attempt in range(1, 4):
            ...
            if uploaded.get("photobank_url") and uploaded.get("file_id") and not client.api_error(response):
                rows.append({**item, "status": "uploaded", "url": str(uploaded["photobank_url"]), "file_id": str(uploaded["file_id"]), **refs(client, response)})
    ...
    existing["assets"] = rows
    existing["counts"] = {role: sum(row["role"] == role for row in rows) for role in ("main", "detail", "company", "sku")}
