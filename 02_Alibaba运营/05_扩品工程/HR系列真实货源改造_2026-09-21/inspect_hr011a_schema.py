"""Read-only HR011-A product/schema inventory for the NK25 candidate migration."""
...
OUTPUT = Path(__file__).with_name("HR011-A_NK25_schema_inventory.json")
PRODUCT_ID = 1601943501378
    ...
    for field in sale_holder.findall(".//field"):
        ...
        for option in options:
            ...
            if name == "Color" and (display or "").lower() not in {
                "black", "grey", "gray", "pink", "black & white", "black/white",
            }:
                continue
...
data = {
    "mode": "read-only",
    "target_family": "HR011 / NK25 candidate",
