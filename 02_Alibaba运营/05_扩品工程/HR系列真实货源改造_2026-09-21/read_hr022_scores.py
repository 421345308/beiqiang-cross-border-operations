"""Read-only Alibaba quality-score diagnostic for the three HR022 links."""
...
def main() -> None:
    ...
    output = Path(__file__).with_name("HR022_质量分_只读回执.json")
    output.write_text(json.dumps(receipt, ensure_ascii=False, indent=2), encoding="utf-8")
    print(json.dumps({
        "output": str(output),
        "request_ids": {listing: row["request_id"] for listing, row in receipt["products"].items()},
    }, ensure_ascii=False))
