"""Read-only HR007-A product/schema inventory for the candidate G225 migration."""
...
OUTPUT = Path(__file__).with_name("HR007-A_G225_schema_inventory.json")
PRODUCT_ID = 1601943463020
...
wanted_attribute_values = {
    "lace-up", "lace up", "suede", "faux suede", "textile", "low cut",
    "round toe", "casual", "casual shoes", "men", "male", "china",
}
    ...
    for field in sale_holder.findall(".//field"):
        ...
        for option in options:
            ...
            if name == "EUR Size" and display not in {str(value) for value in range(39, 45)}:
                continue
            if name == "Color" and (display or "").lower() not in {"black", "yellow", "gray", "grey"}:
                continue
...
data = {
    "mode": "read-only",
    "target_family": "HR007 / G225 candidate",
