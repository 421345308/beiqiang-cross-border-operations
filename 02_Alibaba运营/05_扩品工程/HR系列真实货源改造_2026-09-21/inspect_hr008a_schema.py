"""Read-only HR008-A product/schema inventory for the candidate LT115 migration."""
...
OUTPUT = Path(__file__).with_name("HR008-A_LT115_schema_inventory.json")
PRODUCT_ID = 1601943348656
...
wanted_attribute_values = {
    "lace-up", "lace up", "synthetic", "pu", "low cut",
    ...
    for field in sale_holder.findall(".//field"):
        ...
        for option in options:
            ...
            if name == "EUR Size" and display not in {str(value) for value in range(36, 46)}:
                continue
            if name == "Color" and (display or "").lower() not in {"white", "black", "brown"}:
                continue
...
data = {
    "mode": "read-only",
    "target_family": "HR008 / LT115 candidate",
