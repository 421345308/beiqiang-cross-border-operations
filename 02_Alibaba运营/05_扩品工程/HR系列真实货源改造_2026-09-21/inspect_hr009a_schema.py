"""Read-only HR009-A product/schema inventory for the candidate A2510W migration."""
...
OUTPUT = Path(__file__).with_name("HR009-A_A2510W_schema_inventory.json")
PRODUCT_ID = 1601943434630
...
wanted_attribute_values = {
    "lace-up", "lace up", "mesh", "textile", "low cut",
    ...
    for field in sale_holder.findall(".//field"):
        ...
        for option in options:
            ...
            if name == "Color" and (display or "").lower() not in {
                "black", "white green", "white & green", "white/green",
...
data = {
    "mode": "read-only",
    "target_family": "HR009 / A2510W candidate",
