"""Read-only HR004-A product/schema inventory for the planned 731 migration."""
...
OUTPUT = Path(__file__).with_name("HR004-A_731_schema_inventory.json")
PRODUCT_ID = 1601943397351
...
wanted_attribute_values = {
    ...
    "casual shoes", "men", "male", "china", "light weight", "lightweight",
}
...
if sale_holder is not None:
    for field in sale_holder.findall(".//field"):
        ...
        for option in options:
            ...
            if name == "EUR Size" and display not in {str(value) for value in range(39, 48)}:
                continue
            if name == "Color" and (display or "").lower() not in {"black", "gray", "grey"}:
                continue
