"""Guarded correction of unverified 999 inventory on HR023 A/B/C.

Default is read-only. --apply subtracts only observed 999 values and then
reads every SKU back. This never creates positive inventory.
"""
...
def main() -> int:
    ...
    if any(int(row["inventory"]) not in (0, 999) for row in before):
        raise RuntimeError("Unexpected inventory; no write")
