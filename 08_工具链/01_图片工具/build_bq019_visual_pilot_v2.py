...
OUT = ROOT / "02_Alibaba运营/02_单品优化记录/BQ系列持续优化_2026-09-14/BQ019_W1_整页视觉样板_v2_2026-09-16"
...
# Main gallery: every frame has a different job and a different dominant visual.
m1 = grey.copy()
m1.save(MAIN / "M1_light_grey_white_search_hero.jpg", quality=96)

m2 = canvas(); d = header(m2, "COLOR OPTIONS", "FOUR VERIFIED COLORS", "Single-shoe white-background presentation")
for i, (name, im) in enumerate([("BLACK", black), ("CREAM", cream), ("LIGHT GREY", grey), ("WHITE", white)]):
    x, y = 55 + (i % 2) * 570, 225 + (i // 2) * 440
    card(d, (x, y, x + 520, y + 390))
    paste_round(m2, im, (x + 18, y + 18, 484, 300), 18, contain_mode=True)
    text(d, (x + 28, y + 350), name, 24, INK, True)
m2.save(MAIN / "M2_four_color_range.jpg", quality=95)

m3 = canvas(); d = header(m3, "PRODUCT STRUCTURE", "VISIBLE A206 CONSTRUCTION", "Real product image with verified fields only")
paste_round(m3, black, (55, 235, 690, 850), 28, contain_mode=True)
for i, (title, body) in enumerate([
    ("KNIT / TEXTILE UPPER", "Visible knitted texture"),
    ("LACE-UP CLOSURE", "Adjustable lace construction"),
    ("EVA SOLE", "Visible sole profile"),
]):
    y = 285 + i * 245
    card(d, (785, y, 1145, y + 190), fill=WHITE)
    d.ellipse((812, y + 48, 886, y + 122), fill=GREEN)
    text(d, (849, y + 85), f"0{i + 1}", 21, WHITE, True, anchor="mm")
    text(d, (915, y + 48), title, 20, GREEN_DARK, True)
    d.multiline_text((915, y + 88), wrap(d, body, 205, 19), font=font(19), fill=MUTED, spacing=5)
m3.save(MAIN / "M3_black_product_structure.jpg", quality=95)

m4 = canvas(SAND); d = header(m4, "OEM / ODM", "CUSTOMIZATION DISCUSSION", "Confirm the scope before sampling and bulk production")
paste_round(m4, cream, (55, 235, 590, 820), 30, contain_mode=True)
for i, (title, body) in enumerate([
...
m5 = canvas(); d = header(m5, "QUOTATION FAQ", "WHAT HELPS US QUOTE FASTER?", "Send the purchase brief in one message")
paste_round(m5, white, (645, 230, 500, 560), 28, contain_mode=True)
for i, (title, body) in enumerate([
    ...
    x = 55 if i < 2 else 620
    y = 245 + (i % 2) * 275 if i < 2 else 820
    if i == 3:
        x, y = 620, 820
    if i == 2:
        x, y = 55, 820
    card(d, (x, y, x + 525, y + 220), fill=WHITE)
    text(d, (x + 28, y + 32), title, 25, GREEN, True)
    text(d, (x + 28, y + 87), body, 20, MUTED)
m5.save(MAIN / "M5_white_quotation_faq.jpg", quality=95)

m6 = canvas(GREEN_DARK); d = header(m6, "FACTORY PROFILE", "FROM DEVELOPMENT TO ORDER REVIEW", "Quanzhou footwear manufacturing support", dark=True)
paste_round(m6, factory_exterior, (55, 240, 350, 735), 26, focus=(0.55, 0.52))
paste_round(m6, factory_workshop, (425, 240, 720, 355), 26, focus=(0.52, 0.52))
paste_round(m6, factory_qc, (425, 620, 720, 355), 26, focus=(0.50, 0.58))
for x, value in [(55, "QUANZHOU BASE"), (425, "WORKSHOP"), (790, "ORDER-BASED REVIEW")]:
    text(d, (x, 1040), value, 19, "#DDE9E4", True)
...
with (OUT / "README.md").open("w", encoding="utf-8") as f:
    f.write("# BQ019-W1 整页视觉样板 v2\n\n")
    f.write("状态：仅供视觉复核，未上传平台。\n\n")
    f.write("- 首图：浅灰单鞋、纯白底，不叠字。\n")
    f.write("- 四色：黑 / 米白 / 浅灰 / 白，均为单鞋白底。白色为实拍手工抠图，未使用生成式重画。\n")
    f.write("- 六张主图：搜索首图、四色、结构、OEM/ODM、询价FAQ、工厂概览各司其职。\n")
    f.write("- 工厂：外观、车间、检查使用实拍锚定的 AI 修复候选；第三方字样纸箱图已淘汰。\n")
    f.write("- 淘汰：白鞋 AI 重画（鞋型漂移）；鞋楦货架 AI 修复（数量复制）。\n")
