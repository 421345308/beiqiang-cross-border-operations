# CompShare operation reference

These examples are conditional references, not commands to run as a batch. The installed CLI structured help takes precedence. Read only the section needed for the requested operation.
...
Review the returned selection, capacity, price and request against the authorized scope. When that exact resource and cost are already authorized, rerun without `--dry-run` and add `--yes` plus an explicit timeout; ask only for missing authorization:
...
Treat image deletion, disk deletion, disk detach/resize, quota changes and team mutations as state-changing operations. Read the current resource and confirm that the requested change is within the user-authorized scope before adding `--yes` where supported.
